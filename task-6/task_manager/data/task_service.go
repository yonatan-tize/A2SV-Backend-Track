...
func UpdateTaskById(id primitive.ObjectID, updatedTask models.Task) (*models.Task, error) {
    ...
    err := taskCollection.FindOneAndUpdate(context.TODO(), filter, update, opts).Decode(&updated)
    if err != nil {
        if err == mongo.ErrNoDocuments {
            return nil, err
        }
